...
** Last update Mon Feb  9 22:36:52 2015 Jean-Baptiste Grégoire
*/
...
{
  if ((*free_list = sbrk(21 * MALLOC_PAGE_SIZE + sizeof(t_header))) == (void *)(-1))
    {
      perror("malloc_init");
      return (-1);
    }
  (*free_list)->size = 21 * MALLOC_PAGE_SIZE;
...
int		add_new_page(t_header **free_list)
{
  ...
  if ((new_page = sbrk(21 * MALLOC_PAGE_SIZE + sizeof(t_header))) == (void *)(-1))
    {
      perror("add_new_page");
      return (-1);
    }
  ...
  while (it)
	...
	{
	  it->size = it->size + (21 * MALLOC_PAGE_SIZE) + sizeof(t_header);
	  return (0);
	}
      it = it->next;
    }
  new_page->addr = (void *)((size_t)(new_page) + sizeof(t_header));
  new_page->size = 21 * MALLOC_PAGE_SIZE;
  new_page->next = NULL;
